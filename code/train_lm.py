...
flags.DEFINE_boolean(
    "output_vectors",
    False,
    "output sentence/delta vectors or not")
FLAGS = flags.FLAGS


def load_input_feed(batch, model, state, batch_size):
    input_feed = {model.initial_state: state}

    for i in range(model.para_len):
        targets = batch['sents'][:, i, :]
        targets = targets[:, 1:]
        targets = np.reshape(np.array([list(t) + [2]
                                       for t in targets]), (batch_size, -1))

        input_feed[model.xmiddles[i].name] = batch['sents'][:, i, :]
        input_feed[model.targets[i].name] = targets
        input_feed[model.seq_lens[i].name] = batch['seq_lens'][:, i]

    return input_feed


def run(session, model, reader, epoch, verbose=False):
    ...
    for batch_id, batch_num, word_cnt, batch in reader.yieldSpliceBatch(
            model.mode, model.batch_size, model.step_size, para_len=model.para_len, model_name=FLAGS.model_name, shuffle=True):

        # print ' '.join(['%s:[%s]'%(k,'x'.join([str(s) for s in v.shape])) for
        # k,v in batch.items() if type(v)!=int ])
        state = model.get_initial_state()
        input_feed = load_input_feed(batch, model, state, model.batch_size)

        # TODO add hidden states
        # loss,loss_lm,loss_crf, _ = session.run(
        #        [model.loss,model.loss_lm,model.loss_crf, model.train_op], input_feed)
        loss, _, summary = session.run(
            [model.loss, model.train_op, model.summary_op], input_feed)

        model.summary_writer.add_summary(
            summary, epoch * batch_num + batch_id)

        total_loss += loss
        total_word_cnt += word_cnt
        ppl = np.exp(total_loss / total_word_cnt)
        ...
        perp_summary = tf.Summary(
            value=[
                tf.Summary.Value(
                    ...
                    simple_value=ppl)])
        model.summary_writer.add_summary(
            perp_summary, epoch * batch_num + batch_id)

        progress(
            batch_id * 1.0 / batch_num,
            'loss: %.2f ppl: %.2f speed: %.0f wps words %d' %
                (total_loss / total_word_cnt, ppl, wps, total_word_cnt))
    print()
    return total_loss, total_word_cnt, np.exp(total_loss / total_word_cnt)
...
def train():
    ...
    with tf.Session(graph=graph, config=tf_config) as sess:
        ...
        #valid_model.summary_writer = train_model.summary_writer
        #test_model.summary_writer = train_model.summary_writer
        ...
        for epoch in range(train_model.global_step.eval(), config.epoch_num):
            ...
            # train
            cost, word_cnt, ppl = run(sess, train_model, reader, epoch)
            INFO_LOG(
            ...
            # valid
            cost, word_cnt, ppl = run(sess, valid_model, reader, epoch)
            INFO_LOG(
            ...
            # test
            cost, word_cnt, ppl = run(sess, test_model, reader, epoch)
            INFO_LOG(
        ...
        for level in range(model.para_len - 2 - 1):
            ...
            for middle in range(model.para_len - 2):
                ...
                for pid in range(model.step_size):
                    ...
                    # np.reshape(, [model.batch_size, model.step_infer_size,
                    # -1])
...
def test_bridging(session, model, reader, verbose=False, fwrite=False):
    ...
    for m, v in results.items():
        ...
        # TODO output
#         context_filename = os.path.join(FLAGS.model_path, '%s_%s_gen.txt'%(FLAGS.model_name, FLAGS.dataset))
        # reference_filename = os.path.join(FLAGS.model_path, '%s_%s_gen.txt'%(FLAGS.model_name, FLAGS.dataset))
 #        hypothesis_filename = os.path.join(FLAGS.model_path, '%s_%s_gen.txt'%(FLAGS.model_name, FLAGS.dataset))




def output_vectors(session, model, reader, verbose=False, fwrite=False):
...
def test():
    ...
    with tf.Session(graph=graph, config=tf_config) as sess:
        ...
        if FALGS.output_vectors:
            output_vectors(sess, test_model, reader, verbose=True)
        else:
